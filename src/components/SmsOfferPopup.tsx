      ...
      {open && (
        <div className="pointer-events-none fixed bottom-0 left-0 right-0 z-40 pb-[env(safe-area-inset-bottom)] sm:bottom-5 sm:right-5 sm:left-auto">
          <motion.aside
            ...
            className="pointer-events-auto relative w-full border-y border-border bg-background px-5 pb-5 pt-4 text-left shadow-2xl sm:w-[21rem] sm:border sm:px-6 sm:py-5 sm:text-center"
          >
            <Button
              type="button"
              variant="ghost"
              ...
              className="absolute right-0.5 top-0.5 h-8 w-8 rounded-none text-muted-foreground hover:text-foreground sm:right-1 sm:top-1 sm:h-8 sm:w-8"
            >
              <X aria-hidden="true" />
            </Button>

            {submitted ? (
              <div className="pr-8 py-1 sm:py-2 sm:pr-0">
                <h2 id="sms-offer-title" className="font-heading text-xl uppercase leading-none tracking-[0] text-foreground sm:text-3xl">
                  Check Your Texts
                </h2>
                <p id="sms-offer-description" className="mt-2 text-[11px] leading-4 text-muted-foreground sm:mt-2 sm:text-xs">
              ...
              <>
                ...
                <div className="pr-8 sm:pr-0">
                  <h2 id="sms-offer-title" className="font-heading text-lg uppercase leading-none tracking-[0.02em] text-foreground sm:text-3xl">
                    Get 15% Off
                  </h2>
                  <p id="sms-offer-description" className="mt-2 text-[11px] leading-4 text-muted-foreground sm:mt-2 sm:text-xs">
                ...
                <form onSubmit={handleSubmit} className="mt-4 flex sm:mt-4 sm:w-full">
                  <input
                    type="tel"
                    inputMode="tel"
                    autoComplete="tel"
                    placeholder={isDesktop ? "PHONE NUMBER" : "PHONE NUMBER"}
                    value={phone}
                    onChange={(event) => {
                    ...
                    className="h-11 min-w-0 flex-1 border border-border bg-secondary px-3 text-[11px] uppercase tracking-[0.06em] text-foreground placeholder:text-muted-foreground focus:border-foreground focus:outline-none sm:h-10 sm:px-3 sm:text-[10px] sm:tracking-[0.08em]"
                  />
                  <Button
                    type="submit"
                    disabled={isSubmitting}
                    className="h-11 shrink-0 rounded-none px-4 text-[10px] uppercase tracking-[0.12em] sm:h-10 sm:px-4"
                  >
                ...
                {error && (
                  <p id="sms-offer-error" className="mt-2.5 text-[9px] uppercase leading-4 tracking-[0.1em] text-accent sm:mt-2 sm:text-[10px]" role="alert">
                    {error}
                  </p>
                )}

                <p className="mt-3.5 text-[9px] leading-[14px] text-muted-foreground sm:mt-4 sm:pr-0 sm:leading-3">
